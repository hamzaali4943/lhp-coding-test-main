...
/**
 * Resolves a latitude/longitude into a human-readable city, country and IANA
 * timezone.
 *
 * The seeder scatters every event within ±0.5° of one of a fixed set of city
 * anchors (see EventSeeder::CITY_ANCHORS). Rather than call an external
 * geocoding API 1.25M times, we reverse the same process: snap each coordinate
 * to its nearest known anchor. This is fully offline, deterministic and fast.
 */
class CityResolver
{
    /**
     * [latitude, longitude, city, country, IANA timezone].
     *
    ...
    /**
     * Resolve coordinates to the nearest known city.
     *
     * @return array{city: string, country: string, timezone: string}
     */
    public function resolve(float $latitude, float $longitude): array
    {
        ...
        foreach (self::CITIES as $index => $city) {
            // Squared euclidean distance — monotonic with real distance, and we
            // only need the nearest, so skipping sqrt is fine.
            $dLat = $latitude - $city[0];
            $dLng = $longitude - $city[1];
        ...
        [, , $name, $country, $timezone] = self::CITIES[$bestIndex];

        return ['city' => $name, 'country' => $country, 'timezone' => $timezone];
    }

    /**
     * The distinct list of cities, for filter facets.
     *
    ...
    public static function all(): array
    {
        return array_values(array_map(
            fn (array $c) => ['city' => $c[2], 'country' => $c[3], 'lat' => $c[0], 'lng' => $c[1]],
            self::CITIES,
        ));
    }
}
